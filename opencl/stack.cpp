...
void wkr_thread(const InputData& i, SemblanceHostResult& h, SharedQueue& q, cl_platform_id p_id, cl_device_id dv_id) {
    ...
    SemblanceOpenCl sembl(i, THREADS_PER_BLOCK, ctx);
    SemblanceOpenClParameter dv_prmtr(ctx);
    SemblanceOpenClResult dv_rslt(ctx);
    Stack<SemblanceOpenCl, SemblanceOpenClParameter, SemblanceOpenClResult> s;
    ...
    while (q.dequeue(idx_m0)) {
        ...
        h.save(s.get(Result::N), idx_m0, HostResult::N);
...
int main(int argc, char *argv[]) {
    ...
    hst_rslt.create_all(in.arguments.chosen_method, MethodStrategy::STACK, static_cast<uint32_t>(in.gather.cdps.size()), in.gather.nsmpl_per_trce);
    ...
    hst_rslt.write(in, elpsd_time, MethodStrategy::EXHAUSTIVE, MethodType::OPENCL);
