...
public:
    worker(int argc, const char *argv[], uint32_t id) : ctx(id), dv_rslt(ctx), dv_prmtr(ctx), sembl(in, ctx), dv_id(id)
    {
        in.read(argc, const_cast<char**>(argv));
        ctx.compile(in.arguments.opencl_src);
        nat = new NatureGpuOpenCl(in.arguments.gen, in.arguments.pop_size, in.gather.nsmpl_per_trce, ctx, in.arguments.opencl_src);
...
public:
    ...
    spitz::committer *create_committer(int argc, const char *argv[],
        spitz::istream& jobinfo)
    {
        return new SemblanceCommitter(argc, argv, jobinfo, MethodStrategy::GA);
    }
};
